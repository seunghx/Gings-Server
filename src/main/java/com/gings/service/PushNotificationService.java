 ...
 * 동기화가 가능.)<br>
 * <br>
 * 이런 로직을 수행하기 위해 {@link com.gings.domain.Notification}을 정의하였으며 이 클래스 관련 처리 로직을
 ...
 * <br>
 * 인터페이스 이름을 간단히 NotificationService라고 하지않고 아래와 같이 PushNotificationService라고 정의한 이유는
 * notification은 문자, 메일로도 가능한데 이런 문자, 메일 알림은 푸시 알림같이 따로 관리할 필요는 없기 때문이다.<br>
 * 
 */
